...
contract DAOLanders is Ownable {
  IDAOLandersNFT DAOLandersNFT;
  IDAOLandersMarket DAOLandersMarket;

  constructor(address nftContract, address marketContract) {
    DAOLandersNFT = IDAOLandersNFT(nftContract);
    DAOLandersMarket = IDAOLandersMarket(marketContract);
  }
  ...
  function createProposal(uint256 listingId, bool buy) public {
    ...
    if (buy) {
      ...
      _daoProposal.listing.nftTokenId = DAOLandersMarket.getNftId(listingId);
      _daoProposal.listing.daoID = DAOLandersMarket.getDAOID(listingId);
  ...
  function executeProposal() public {
    ...
    if (_daoProposal.proposalType == ProposalType.BUY) {
      if (_daoProposal.acceptVotes > _daoProposal.rejectVotes) {
        ...
        if (
          DAOLandersMarket.checkListingExpiry(_daoProposal.listing.listingId)
        ) {
          //Swapping in NFT contract
          DAOLandersNFT.nftSwapOwnership(
          ...
          DAOLandersMarket.expireListing(_daoProposal.listing.listingId);
        } else {
      ...
      DAOLandersMarket.createListing(_dao.NFTId, userToDAO[msg.sender]);
    }
  ...
  function onERC1155Received(
    ...
    uint256 _daoID = DAOLandersNFT.nftOwnershipCheck(tokenId);
